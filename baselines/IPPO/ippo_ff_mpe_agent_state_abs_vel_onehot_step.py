...
This controlled variant keeps the previous absolute-velocity + one-hot input
and adds exactly one new feature: normalized episode step.
...
Each shared policy receives:
  ...
  - normalized episode step = state.step / env.max_steps

For the default 3-agent / 3-landmark SimpleSpread task, the input is 22-D.
...
def build_policy_input(mode, obs, env_state, env, config):
    if mode == "agent_centric_state_abs_vel_onehot_step":
...
def make_train(config):
    ...
    if input_mode != "agent_centric_state_abs_vel_onehot_step":
        raise ValueError(
            "This script expects INPUT_MODE='agent_centric_state_abs_vel_onehot_step', "
            f"got {input_mode}"
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="ippo_ff_mpe_agent_state_abs_vel_onehot_step",
)
def main(config):
    ...
    print(
        "IPPO FF agent-centric state (absolute other velocity + one-hot ID + normalized step) | "
