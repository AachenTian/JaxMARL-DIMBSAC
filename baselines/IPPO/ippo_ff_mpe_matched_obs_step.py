"""
Matched feed-forward IPPO using the standard SimpleSpread observation plus
one scalar normalized episode-step feature.

This is a controlled diagnostic against ippo_ff_mpe_matched:
    matched obs       : policy input = o_i
    matched obs+step  : policy input = [o_i, state.step / env.max_steps]

Everything else is unchanged:
    - matched-FF network architecture
    - PPO hyperparameters
    - trajectory-wise minibatching
    - seed
    - W&B update/env-step semantics

For default SimpleSpread, the standard observation is 18-D, so this variant
uses a 19-D policy input.
"""
...
def build_local_obs_step_batch(
    obs,
    log_env_state,
    env,
    config,
):
    """Append normalized episode progress to each agent's standard observation.

    The same scalar t / T is repeated for all agents in a given environment.
    Output ordering stays agent-major, matching batchify().
    """
    obs_batch = batchify(
        obs,
        env.agents,
    ...
    normalized_step = jnp.tile(
        ...
        (env.num_agents, 1, 1),
    ).reshape((config["NUM_ACTORS"], 1))

    return jnp.concatenate(
        [obs_batch, normalized_step],
        axis=-1,
    )
...
def build_policy_input(mode, obs, env_state, env, config):
    ...
    if mode == "local_step":
        return build_local_obs_step_batch(
            obs,
            env_state,
            env,
            config,
...
def make_train(config):
    ...
    if input_mode == "local":
        ...
        raise ValueError(
            "INPUT_MODE must be 'local', 'local_step', or "
            f"'full_state', got {input_mode}"
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="ippo_ff_mpe_matched_obs_step",
)
def main(config):
