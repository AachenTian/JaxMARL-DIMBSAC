...
def build_policy_input(mode, obs, env_state, env, config):
    if mode == "agent_centric_state_abs_vel":
...
def make_train(config):
    ...
    if input_mode != "agent_centric_state_abs_vel":
        raise ValueError(
            "This script expects INPUT_MODE='agent_centric_state_abs_vel', "
            f"got {input_mode}"
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="ippo_ff_mpe_agent_state_abs_vel",
)
def main(config):
    ...
    print(
        "IPPO FF agent-centric state (absolute other velocity) | "
