...
def full_state_input_dim(env):
    ...
    # p_pos, p_vel, c, step, plus controlled-agent identity.
    physical_dim = (
        env.num_entities * env.dim_p
        + env.num_entities * env.dim_p
        + env.num_agents * env.dim_c
        + 1  # normalized episode step
    )
...
def build_full_state_batch(log_env_state, env, num_envs):
    ...
    For 3-agent / 3-landmark SimpleSpread this is:
      ...
      normalized step: 1
      agent ID: 3
      total: 34 dimensions per actor.

    The terminal flag is intentionally omitted. In MPE it is determined by
    the episode step and is therefore redundant once normalized step is present.
    Omitting it also avoids assuming whether a local JaxMARL checkout stores
    `state.done` as a scalar or a per-agent vector.
    """
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="ippo_ff_mpe_full_state",
)
def main(config):
