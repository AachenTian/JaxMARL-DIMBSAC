...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            ...
            def callback(metric):
                # Match the RNN/matched-run W&B semantics.
                wandb.log(
                    {
            ...
            loss_info = {**loss_info, **r0}

            callback_metric = jax.tree.map(
                lambda x: x.reshape(
                    (config["NUM_STEPS"], config["NUM_ENVS"], env.num_agents)
                ),
                traj_batch.info,
            )
            callback_metric["loss"] = loss_info
            callback_metric["update_steps"] = update_steps
            jax.experimental.io_callback(callback, None, callback_metric)

            # Preserve the original FF scan-output format for the local plot code.
        ...
        (runner_state, _), metric = jax.lax.scan(
            _update_step,
            (runner_state, jnp.array(0, dtype=jnp.int32)),
            None,
            config["NUM_UPDATES"],
        )
...
@hydra.main(version_base=None, config_path="config", config_name="ippo_ff_mpe_jaxmarl_step_wandb")
def main(config):
