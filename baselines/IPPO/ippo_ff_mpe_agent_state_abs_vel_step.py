...
This differs from the previous abs-velocity version by exactly one scalar:
state.step / env.max_steps.

The matched-FF network, PPO hyperparameters, seed, trajectory-wise
minibatching, and W&B step semantics are unchanged.
"""
...
def make_train(config):
    ...
    if input_mode != "agent_centric_state_abs_vel_step":
        raise ValueError(
            "This script expects INPUT_MODE='agent_centric_state_abs_vel_step', "
            f"got {input_mode}"
        )

    policy_input_dim = agent_centric_state_input_dim(base_env)
    standard_obs_dim = base_env.observation_space(
        base_env.agents[0]
    ).shape[0]
    # The original abs-velocity variant is dimension-matched to the
    # standard 18-D SimpleSpread observation by replacing the 4-D silent
    # communication block with 4-D other-agent absolute velocity.
    # This controlled variant appends one extra normalized-step scalar,
    # so its policy input is expected to be standard_obs_dim + 1.
    expected_policy_input_dim = standard_obs_dim + 1
    if policy_input_dim != expected_policy_input_dim:
        raise ValueError(
            "Unexpected agent-centric input dimension: "
            f"{policy_input_dim}; expected {expected_policy_input_dim} "
            f"(standard observation {standard_obs_dim} + normalized step 1)."
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="ippo_ff_mpe_agent_state_abs_vel_step",
)
def main(config):
    ...
    print(
        "IPPO FF agent-centric state (absolute other velocity + normalized step) | "
