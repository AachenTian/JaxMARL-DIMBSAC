...
Each shared policy receives a different ego-centric physical state:
  ...
  - other-agent velocities relative to ego
...
relative velocities while keeping the network and PPO optimization regime
...
def agent_centric_state_input_dim(env):
    ...
    Layout:
      ...
      other-agent relative velocities:2 * (num_agents - 1)
    """
    return (
        2 * env.dim_p
...
def build_agent_centric_state_batch(log_env_state, env, num_envs):
    ...
    with i removed. Relative velocity is v_j - v_i.
    """
    ...
    for agent_idx in range(env.num_agents):
        ...
        agent_input = jnp.concatenate(
            [
                ...
                other_rel_vel.reshape((num_envs, -1)),
            ],
            axis=-1,
        )
...
def make_train(config):
    ...
    if input_mode != "agent_centric_state":
        raise ValueError(
            "This script expects INPUT_MODE='agent_centric_state', "
            f"got {input_mode}"
        )
    ...
    if policy_input_dim != standard_obs_dim:
        raise ValueError(
            "Agent-centric state and standard observation dimensions differ: "
            f"{policy_input_dim} vs {standard_obs_dim}."
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="ippo_ff_mpe_agent_state",
)
def main(config):
