...
from dynamics.normalization import (
    build_oracle_pos_dynamics_data,
...
EXPERIMENT_NAME = "oracle_pos_13d"
...
def train_agent_ensemble(
    ...
    """
    Train one 13D -> 4D oracle-position independent dynamics ensemble.
    """
    ...
    if dynamics_inputs.shape[-1] != 13:
        raise ValueError(
            "Oracle-position dynamics input must be 13D "
...
def main(
    ...
    try:
        ...
        if num_agents != 3:
            raise ValueError(
                "This 13D oracle-position diagnostic currently "
                f"expects 3 agents, got {num_agents}."
            )

        print(
            "ESTIMATOR: Oracle-position independent dynamics"
        )
        print(
            "=" * 64
        )
        ...
        print(
            ...
            "4D true other relative positions = 13D",
        )
        print(
            "target:",
            "delta_x_i = 4D",
        ...
        print(
            "\n"
            "Oracle-position 13D dynamics experiment finished."
        )
