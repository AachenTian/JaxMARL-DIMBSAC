...
from dynamics.normalization import (
    build_oracle_pos_abs_vel_dynamics_data,
...
EXPERIMENT_NAME = "oracle_pos_absvel_17d"
...
def train_agent_ensemble(
    ...
    """
    Train one 17D -> 4D oracle-position + absolute-velocity dynamics ensemble.
    """
    ...
    if dynamics_inputs.shape[-1] != 17:
        raise ValueError(
            "Oracle-position + absolute-velocity dynamics input must be 17D "
            f"for 3-agent SimpleSpread, got {dynamics_inputs.shape[-1]}D."
        )
...
def main(
    ...
    try:
        env = jaxmarl.make(
            ...
            contact_force=cfg.ENV.CONTACT_FORCE,
        )
        ...
        if num_agents != 3:
            raise ValueError(
                "This 17D oracle-position+absolute-velocity diagnostic currently "
                f"expects 3 agents, got {num_agents}."
            )

        print(
            "ESTIMATOR: Oracle-position + absolute-velocity independent dynamics"
        )
        print(
            "=" * 64
        )
        ...
        print(
            "input:",
            "4D own state + 5D own action + "
            "4D true other relative positions + "
            "4D true other absolute velocities = 17D",
        )
        print(
            "target:",
            "delta_x_i = 4D",
        ...
        agent_total_rmse = []
        agent_per_dim_rmse = []
        agent_per_dim_nrmse_std = []
        agent_per_dim_zero_ratio = []
        agent_per_dim_zero_improvement_pct = []
        ...
        for agent_idx in range(
            ...
            # All ensemble predictions above are already in physical units.
            # target_std comes from the training-target normalization statistics.
            ...
            agent_per_dim_nrmse_std.append(
                per_dim_nrmse_std
            )
            agent_per_dim_zero_ratio.append(
                per_dim_zero_ratio
            )
            agent_per_dim_zero_improvement_pct.append(
                per_dim_zero_improvement_pct
            )
            ...
            print(
                "improvement vs zero-delta baseline (%):",
                {
                    name: float(value)
                    for name, value in zip(
                        DIM_NAMES,
            ...
            if bool(cfg.WANDB.ENABLED):
                ...
                for dim_idx, dim_name in enumerate(
                    ...
                    log_data[
                        (
                            f"dynamics/agent_{agent_idx}/"
                            f"ensemble/improvement_vs_zero_pct_{dim_name}"
                        )
                    ] = float(
                        per_dim_zero_improvement_pct[
                            dim_idx
        ...
        agent_per_dim_nrmse_std = jnp.stack(
            agent_per_dim_nrmse_std,
            axis=0,
        )
        agent_per_dim_zero_ratio = jnp.stack(
            agent_per_dim_zero_ratio,
            axis=0,
        )
        agent_per_dim_zero_improvement_pct = jnp.stack(
            agent_per_dim_zero_improvement_pct,
            axis=0,
        )
        ...
        mean_per_dim_nrmse_std = jnp.mean(
            agent_per_dim_nrmse_std,
            axis=0,
        )
        mean_per_dim_zero_ratio = jnp.mean(
            agent_per_dim_zero_ratio,
            axis=0,
        )
        mean_per_dim_zero_improvement_pct = jnp.mean(
            agent_per_dim_zero_improvement_pct,
            axis=0,
        )

        mean_velocity_nrmse_std = jnp.mean(
            mean_per_dim_nrmse_std[
                2:
            ]
        )
        mean_velocity_zero_ratio = jnp.mean(
            mean_per_dim_zero_ratio[
                2:
            ]
        )
        ...
        print(
            "mean velocity NRMSE/std:",
            float(mean_velocity_nrmse_std),
        )
        print(
            "mean velocity RMSE / zero-delta baseline:",
            float(mean_velocity_zero_ratio),
        )
        print(
            "mean per-dim RMSE:",
            {
            ...
            wandb.log(
                {
                    ...
                    "summary/mean_velocity_nrmse_std": float(
                        mean_velocity_nrmse_std
                    ),
                    "summary/mean_velocity_zero_ratio": float(
                        mean_velocity_zero_ratio
                    ),
                    ...
                    "summary/mean_nrmse_std_px": float(
                        mean_per_dim_nrmse_std[0]
                    ),
                    "summary/mean_nrmse_std_py": float(
                        mean_per_dim_nrmse_std[1]
                    ),
                    "summary/mean_nrmse_std_vx": float(
                        mean_per_dim_nrmse_std[2]
                    ),
                    "summary/mean_nrmse_std_vy": float(
                        mean_per_dim_nrmse_std[3]
                    ),
                    "summary/mean_zero_ratio_px": float(
                        mean_per_dim_zero_ratio[0]
                    ),
                    "summary/mean_zero_ratio_py": float(
                        mean_per_dim_zero_ratio[1]
                    ),
                    "summary/mean_zero_ratio_vx": float(
                        mean_per_dim_zero_ratio[2]
                    ),
                    "summary/mean_zero_ratio_vy": float(
                        mean_per_dim_zero_ratio[3]
                    ),
                }
            )
        ...
        print(
            "\n"
            "Oracle-position + absolute-velocity 17D dynamics experiment finished."
        )
